...
-- 급여최소값, 가장 빠른 입사일, 알파벳 순서가 가장 빠른 이메일

SELECT MIN(SALARY), MIN(HIRE_DATE), MIN(EMAIL)
FROM EMPLOYEE ;

-- 급여최대값, 가장 느린 입사일,  알파벳 순서가 가장 느린ㄴ 이메일
SELECT MAX(SALARY), MAX(HIRE_DATE), MAX(EMAIL)
FROM EMPLOYEE ;

-- EMPLOYEE 테이블에서 가장 급여를 많이 받는 사원의
-- 이름, 급여, 직급 코드 조회
