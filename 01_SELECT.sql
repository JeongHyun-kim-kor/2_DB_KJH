...
-- SYSDATE : 시스템 상의 현재 시간(날짜) 나타내는 상수
...
-- DB 리터럴 : 임의로 지정한 값을 기본 테이블에 존재하는 값 처럼 사용하는 것
--> (필수) DB의 리터럴 표기법은 ' ' 홑따옴표
--> " "(쌍또옴표) : 특수문자, 대소문자, 기호 등을 구분하여 나타낼 때 사용하는 표기법
...
-- EMPLOYEE 테이블에서 부서코드가 'D0'인 사원의
-- 사번, 이름, 부서코드, 직급코드 조회

SELECT EMP_ID , EMP_NAME , DEPT_CODE , JOB_CODE 
FROM EMPLOYEE
WHERE DEPT_CODE = 'D9'; -- 비교연산자 (=), 대입 연산자(:=)
...
-- NOT BETWEEN A AND B : A 이상 B 이하 가 아닌 경우!
...
-- 'A%' : A로 시작하는 문자열
-- '%A' : A로 끝나는 문자열
-- '%A%' : A를 포함하는 문자열 처음에 잇어도 가능?

-- _ 예시
-- 'A_' : A로 시작하는 두 글자 문자열

-- '___A' : A로 시작하는 네글자 문자열
...
FROM EMPLOYEE 
WHERE EMAIL LIKE '____%';  -- 4글자 이상인 이메일 조회가 된다.
							--   _ 가 인식이 안된다
--> _를 기준점으로 삼았으나 와일드 카드 '_'와 동일한 표기법으로 작성되어
-- 구분되지 않음
...
SELECT EMP_NAME , EMAIL 
FROM EMPLOYEE 
WHERE EMAIL LIKE '___#_%' ESCAPE '#';  -- #, ^ 등
-- #뒤에오는 _ 는 일반 문자로 탈출시킨다.

-- 연습문제
-- EMPLOYEE 테이블에서 
-- 이메일 '_' 앞이 4글자 이면서 
-- 부서 코드가 'D9' 또는 'D6'이고   --> AND가 OR보다 우선순위가 높다.
...
OR DEPT_CODE IS NULL ; -- 2명(NULL 처리 연산자)  >> 총 14명
...
-- EMPLOYEE 테이블에서 보너스가 있는 사원의 이름, 보너스조회
	SELECT EMP_NAME, BONUS 
	FROM EMPLOYEE 
	WHERE BONUS IS NOT NULL; -- 9행

-- EMPLOYEE 테이블에서 보너스가 없는 사원의 이름, 보너스조회
