...
/* 자료형
    ...
    DATE : 날짜 타입
    BLOB : 대용량 이진 데이터 (4GB) BYTE LONG OF  > 지양
    CLOB : 대용량 문자 데이터 (4GB)
*/
...
-- 문자인코딩 UTF-8 : 영어, 숫자 = 1BYTE / 한글, 특수문자 등은 3 BYTE 취급
...
--> 둘 다 컬럼의 값을 지정된 형태로만 제한하지만 
-- CHECK는 작성 후 수정, 번거롭지만
-- FK는 부모 테이블에 행을 추가, 수정, 삭제 하는게 간편하다
 ----------------------------------------------------------------------------------------------------------------
...
CREATE TABLE USER_TEST (
	USER_NO VARCHAR2(30) CONSTRAINT PK_USER_TEST PRIMARY KEY,
	...
	GENDER VARCHAR2(30) CONSTRAINT CK_GENDER CHECK (GENDER IN('남','여')),
	PHONE VARCHAR2(30)
	ADDRESS VARCHAR2(30)
	STATUS VARCHAR2(30) CONSTRAINT CK_STATUS NOT NULL,
	
);
COMMENT ON COLUMN USER_TEST.USER_NO IS '회원번호';

INSERT INTO USER_TEST VALUES
(1,'USER01', 'PASS01', '950310-1111223','남','010-9874-9078', '경기도 구리시','N');
...
--> 서브쿼리로 테이블 생성 시 
-- 테이블의 형태(컬럼명, 데이터타입) + NOT NULL 제약조건만 복사됨!!
-- 제약조건, 코멘트는 복사되지 않기 때문에 추가 작업이 필요하다.
...
ALTER TABLE LOCATION  ADD CONSTRAINT LOC_NATIONAL_CODE
	FOREIGN KEY(NATIONAL_CODE) REFERENCES LOCATION ON DELETE SET NULL;
