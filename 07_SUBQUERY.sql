...
-- 1) 전 직원의 평균 급여 조회하기
SELECT CEIL(AVG(SALARY))  -- 3047663
FROM EMPLOYEE; 
...
SELECT EMP_ID, EMP_NAME, JOB_CODE, SALARY
FROM EMPLOYEE 
WHERE SALARY >= (SELECT CEIL(AVG(SALARY))  -- 3047663
				FROM EMPLOYEE);
...
-- 1)  MAUN 쿼리의 1행의 DEPT_CODE를 서브쿼리에 대입
-- 2) SUB 수행
-- 3) SUB의 결과를 이용해서 MAIN의 조건절을 수행


SELECT DEPT_TITLE, E.EMP_NAME, E.JOB_CODE , J.JOB_CODE ,J.JOB_NAME 
FROM EMPLOYEE E
LEFT JOIN DEPARTMENT ON(DEPT_CODE = DEPT_ID)
...
-- 7. 인라인 뷰(INLINE-VIEW) FROM절에 쓰는 서브쿼리
...
FROM EMPLOYEE
WHERE ROWNUM <= 5
ORDER BY SALARY DESC;  --> 순서 탑5가 됨 -> 인라인 뷰 사용
--> SELECT 해석 순서때문에 
-- 급여 상위 5명이 아니라 조회 순서 상위 5명의 급여 순위가 조회됨

--> 인라인 뷰를 이용해서 해결 가능!

-- 1) 이름, 급여를 급여 내림차순을 조회한 결과를 
-- 인라인뷰로 사용
...
FROM(SELECT EMP_NAME, SALARY
	FROM EMPLOYEE 
	ORDER BY SALARY DESC)  -- 1순위로 FROM절에서 급여순서대로 내림차순
WHERE ROWNUM <= 5;
...
-- 
-- 전 직원의 급여 순위 
-- 순위, 이름, 급여 조회된 결과(서브쿼리의 결과)를 TOP_SAL이라 부르겠다.
