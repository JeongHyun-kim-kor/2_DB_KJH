/* VIEW
	...
	***** VIEW 사용 시 주의사항 *****
    ...
    [VIEW 생성 방법]
	    ...
	    AS subquery             잘안씀                      잘 안씀
	    [WITH CHECK OPTION]   -- 잘안씀
	    [WITH READ ONLY];
...
CREATE OR REPLACE VIEW V_EMP AS SELECT EMP_ID, EMP_NAME, DEPT_TITLE, JOB_NAME
								FROM EMPLOYEE 
...
AS SELECT EMP_ID 사번, EMP_NAME 이름, DEPT_TITLE 부서명, JOB_NAME 직급명
	...
	JOIN DEPARTMENT ON(DEPT_CODE = DEPT_ID)
	JOIN JOB USING(JOB_CODE);
 -- 기존에 존재하던 VIEW이니 덮어쓰기가 됨

-- ORA-00955 : 기존의 객체가 이름을 사용하고 있습니다. ==> 이름 중복
-- OR REPLACE 추가 후 진행 -> 성공
